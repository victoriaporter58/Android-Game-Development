...
public class PlayerSpaceship extends SpaceEntity {
    ...
    private Emitter mMovementEmitterMid;
    ...
    private Vector2 mMovementEmitterOffsetMid;
    ...
    public PlayerSpaceship(float startX, float startY, GameScreen gameScreen) {
        ...
        mMovementEmitterOffsetMid = new Vector2(-23.0f, -0.0f);
        ...
        // Create and add the mid emitter
        mMovementEmitterLocation.set(position);
        mMovementEmitterLocation.add(mMovementEmitterOffsetMid);
        mMovementEmitterMid = new Emitter(
                particleSystemManager, "txt/particle/ThrusterEmitterMid.JSON",
                mMovementEmitterLocation);
        particleSystemManager.addEmitter(mMovementEmitterMid);
    }
    ...
    public void update(ElapsedTime elapsedTime, ThumbStick movementThumbstick) {
        ...
        MathsHelper.rotateOffsetAboutCentre(
                position, mMovementEmitterOffsetMid, orientation, mMovementEmitterLocation);
        mMovementEmitterMid.setPosition(mMovementEmitterLocation.x, mMovementEmitterLocation.y);
        ...
        if(velocity.length()<=95){
            mMovementEmitterMid.getEmitterSettings().minParticleDensity = 0;
            mMovementEmitterMid.getEmitterSettings().maxParticleDensity =0;
        }else{
            mMovementEmitterMid.getEmitterSettings().minParticleDensity = (int) velocity.length();
            mMovementEmitterMid.getEmitterSettings().maxParticleDensity = (int) (5.2f * velocity.length());
        }

        //Play a sound when the spaceship starts and stops
        if((Math.abs(velocity.length()) <= 95) && (Math.abs(velocity.length()) >= 10)){
            AudioManager audioManager = getGameScreen().getGame().getAudioManager();
            audioManager.play(
                    getGameScreen().getGame().getAssetManager().getSound("SpaceshipSound")
            );
        }
    }
}
